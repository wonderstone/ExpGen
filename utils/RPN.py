...
# Define default supported operators (can be expanded with custom functions)
DEFAULT_OPS = {
    # '+': lambda a, b: a + b,
    # '-': lambda a, b: a - b,
    '+': add,
...
class RPNExpression:
    def __init__(self, expression: List[Union[str, float, int]], custom_ops=None):
        """
    ...
    def validate_expression(self):
        ...
        for token in self.expression:
            if isinstance(token, (int, float)) or token.isdigit() or isinstance(token, str):  # Support variables
                ...
                _, arity, _ = self.ops[token]  # Get the arity of the operator
                if len(stack) < arity:
...
if __name__ == "__main__":
    ...
    rpn_expr = RPNExpression(['x', 'neg', 2, '+'])
    ...
    rpn_ternary_expr = RPNExpression(['x', 'y', 'z', '?'])
    ...
    rpn_power_expr = RPNExpression(['x', 'neg', 2, '+', 3, 'pow'])
